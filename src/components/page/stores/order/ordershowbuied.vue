...
<template>
    <div>
            <el-dialog title="购买商品展示" :visible.sync="goummai" width="38%">
                  <el-table
      :data="contactarrtwo"
  ...
  data() {
    ...
    methods:{ 
      rowspan(){
        ...
        this.contactarrtwo.forEach((item,index) => {
          if( index === 0){
            this.spanArr.push(1);
            this.position = 0;
            // item.sequence=index+1;//设置序号
          }else{
            if(this.contactarrtwo[index].goodState === this.contactarrtwo[index-1].goodState ){
    ...
    computed:{
         ...mapState(["goummai","contactarr","contactarrtwo"]),
     goummai: {
      get() {
        return this.$store.state.store.goummai;
      },
      set(goummai) {
        this.$store.commit("store/setgoummai", goummai);
      }
    },
    }
